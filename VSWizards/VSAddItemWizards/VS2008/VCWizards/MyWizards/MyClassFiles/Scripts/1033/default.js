...
function OnFinish(selProj, selObj)
{
	try
	{
		...
		if (!wizard.ParentObject.ValidateMember(className, vsCMElementClass))
		{
			wizard.ReportError("The class name \"" + className + "\" is illegal. \nPlease choose another class name.");
			return VS_E_WIZARDBACKBUTTONPRESS;
		}

		var headerName = className + ".h";
		if (!selProj.Object.CanAddFile(headerName))
		{
			wizard.ReportError("The file \"" + headerName + "\" already exists. \nPlease choose another class name.");
			return VS_E_WIZARDBACKBUTTONPRESS;
		}

		var sourceName = className + ".cpp";
		if (!selProj.Object.CanAddFile(sourceName))
		{
			wizard.ReportError("The file \"" + sourceName + "\" already exists. \nPlease choose another class name.");
			return VS_E_WIZARDBACKBUTTONPRESS;
		}
		...
		var headerFileGuarder = headerName.replace(/\./g, "_").toUpperCase() + "_GUID_" + strFormattedGUID.replace(/-/g, "_");
		...
		var timeStr = (date.getHours() + 100 + "").substr(1) + ":" + (date.getMinutes() + 100 + "").substr(1) + ":" + (date.getSeconds() + 100 + "").substr(1);
		wizard.AddSymbol("CREATE_DATE", dateStr);
		wizard.AddSymbol("CREATE_TIME", timeStr);
...
{
	if (strFileName.toLowerCase() == "headerFile.h".toLowerCase())
		oFileItem.Object.FileType = eFileTypeCppHeader;
	else if (strFileName.toLowerCase() == "sourceFile.cpp".toLowerCase())
		oFileItem.Object.FileType = eFileTypeCppCode;
...
function GetTargetName(strName, strProjectName, strResPath, strHelpPath)
{
	try
	{
		...
		if (strName.toLowerCase() == "headerFile.h".toLowerCase())
		    strTarget = wizard.FindSymbol("HEADER_FILE");
		else if (strName.toLowerCase() == "sourceFile.cpp".toLowerCase())
		    strTarget = wizard.FindSymbol("SOURCE_FILE");
