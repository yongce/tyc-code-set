...
function OnFinish(selProj, selObj)
{
	try
	{
		var filename = wizard.FindSymbol("ITEM_NAME");
		var suffix = filename.substr(filename.length - 2);
		if (suffix.toLowerCase() != ".h")
			filename = filename + ".h";

		if (!selProj.Object.CanAddFile(filename))
		{
			ReportError("The file \"" + filename + "\" already exists. \nPlease choose another file name.");
			return VS_E_WIZARDBACKBUTTONPRESS;
		}

		wizard.AddSymbol("FILENAME", filename);
